...
def level_select(n):
    ...
    if len(files)==0:
        ...
        draw_texts(screen, ["To many levels in /Map directory","Maximum amount is 20"], (SIZE[0]/2,SIZE[1]/5), 20, WHITE, spacing=30)
    else:
        draw_texts(screen, files[:int((len(files)+1)/2)], (SIZE[0]/4,SIZE[1]/8+50), 20, WHITE, spacing=40)
        draw_texts(screen, files[int((len(files)+1)/2):], (3*SIZE[0]/4,SIZE[1]/8+50), 20, WHITE, spacing=40)
